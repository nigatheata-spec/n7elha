...
- **Paint Fight** (`paintfight`) — free-for-all territory capture (paper.io / splix style), `paintFight.ts` / `paintFightRender.ts`. Every player owns a patch of ground. You drive out of it with a virtual joystick leaving a trail, and closing a loop back onto your own ground claims everything it encloses, taking it off whoever held it. You are only vulnerable outside your own territory: anyone touching your trail (or you touching it yourself, or hitting the arena wall) wipes everything you hold and respawns you on a fresh block. A colour tank drains as you move and only a correct answer refills it — no power-ups and no per-question timer, deliberately just the core loop. Synced by an append-only log over Supabase (`paint_fight_strokes`) with two ops, `claim` and `wipe`, replayed by every client with last-write-wins per cell; see that file's header for why cell-index batches are logged instead of raw pixels, and for the rule that each client is authoritative over its own capture and its own death and nothing else. Cell ownership is the *single* source of truth: territories are drawn from each player's cell set (collapsed to horizontal runs, filled and round-join-stroked as a cached world-space `Path2D` — flat vector shapes, no pixelation), so what's rendered is literally the score.
