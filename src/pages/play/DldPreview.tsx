...
const DldPreview = () => {
  ...
  const climb = useMemo(
    () => buildClimb(Number(new URLSearchParams(window.location.search).get("min") ?? 10)), []);
  ...
  const nearest = climb.platforms.reduce((a, b) => (Math.abs(b.y - y0) < Math.abs(a.y - y0) ? b : a), climb.platforms[0]);
  ...
  useEffect(() => {
    ...
    const frame = (t: number) => {
      ...
      const blend = themeBlendAt(climb, cam.current.y);
      drawSky(ctx, bw, bh, blend);

      const ti = themeIndexAt(climb, cam.current.y);
      drawStars(ctx, bw, bh, camX * PX, camY * PX, starAlphaAt(climb, cam.current.y));

      const cloudAlpha = ti > STARRY_FROM ? 0 : 1;
      if (cloudAlpha > 0) forEachCloud(camBX, camBY, bw, bh, (px, py, c) => {
